...
import {
  ...
  StrategyData,
  BacktestResult,
} from "@/lib/services/backtestJobs";
...
async function processBacktestStrategy(
  ...
  for (let i = 0; i < points; i++) {
    mockResults.equityCurve.push({
      time: Math.floor(
        (Date.now() - (points - i) * 24 * 60 * 60 * 1000) / 1000
...
export async function POST(request: Request) {
  ...
  try {
    ...
    processBacktestStrategy(strategyData, jobId).catch((error) => {
      // Background backtest processing error handling
...
export async function GET() {
  ...
  return NextResponse.json({
    ...
    usage:
      "POST strategy data to this endpoint to run a backtest, or GET /api/backtests/{jobId} to retrieve results",
  });
}
