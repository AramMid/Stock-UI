...
export class DrawingService {
  ...
  private getCursorStyle(): string {
    ...
    const cursorOption = (window as any).cursorOptions?.find(
      (opt: any) => opt.id === this.cursorType
    );
  ...
  private handleMultiPointDrawing(
  ...
  private updatePolygonPoints() {
    ...
    const pointsString = this.polygonPoints
      .map((p) => `${p.x},${p.y}`)
      .join(" ");
  ...
  private handleMouseMove(e: MouseEvent) {
    ...
    if (this.isDragging && this.selectedElement && this.dragOffset) {
      ...
      const newX2 =
        e.offsetX -
        this.dragOffset.x +
        (parseFloat(this.selectedElement.getAttribute("x2") || "0") -
          parseFloat(this.selectedElement.getAttribute("x1") || "0"));
      const newY2 =
        e.offsetY -
        this.dragOffset.y +
        (parseFloat(this.selectedElement.getAttribute("y2") || "0") -
          parseFloat(this.selectedElement.getAttribute("y1") || "0"));
  ...
  private createDrawingElement(tool: string): SVGElement | null {
    const id = `drawing-${Date.now()}-${Math.random()
      .toString(36)
      .substr(2, 9)}`;
  ...
  private addHoverEffects(element: SVGElement) {
    element.addEventListener("mouseenter", () => {
      ...
      element.setAttribute(
        "stroke-width",
        (parseInt(currentStrokeWidth) + 1).toString()
      );
    });

    element.addEventListener("mouseleave", () => {
      const originalStrokeWidth =
        element.getAttribute("data-original-stroke-width") || "2";
    ...
    switch (element.tagName.toLowerCase()) {
      ...
      case "polygon":
        ...
        if (tool === "triangle") {
          ...
          element.setAttribute(
            "points",
            `${start.x},${start.y} ${end.x},${end.y} ${midX},${midY}`
          );
        } else if (tool === "arrow") {
          // Create arrow shape
          ...
          const leftWingX =
            arrowHeadX + headWidth * Math.cos(angle - Math.PI / 2);
          const leftWingY =
            arrowHeadY + headWidth * Math.sin(angle - Math.PI / 2);
          ...
          element.setAttribute(
            "points",
            `${start.x},${start.y} ${arrowHeadX},${arrowHeadY} ${leftWingX},${leftWingY} ${end.x},${end.y} ${rightWingX},${rightWingY} ${arrowHeadX},${arrowHeadY}`
          );
        } else {
  ...
  toggleVisibility() {
    if (this.svgOverlay) {
      this.svgOverlay.style.display =
        this.svgOverlay.style.display === "none" ? "" : "none";
    }
  }
}
