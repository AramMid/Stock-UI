...
function formatShares(n: number) {
  console.log('Formatting shares:', n, 'Type:', typeof n);
...
export default function RightSidebar({
  ...
  console.log('WatchlistSection received positions:', positions);
  console.log('WatchlistSection positions map contents:', Array.from(positions.entries()));
  ...
  const getMarketSimulation = useCallback(() => marketSimulation || null, [marketSimulation]);
  ...
  const stockWatchlist: WatchlistItem[] = stockSymbols.map((symbol) => {
    ...
    console.log(`Processing symbol ${symbol}: position = ${position}, map has:`, positions.has(symbol));
    if (symbol === 'VIC.VN') {
      console.log(`VIC.VN specific debug: positions.get('VIC.VN') =`, positions.get('VIC.VN'));
      console.log(`VIC.VN positions map contents:`, Array.from(positions.entries()));
      console.log(`All stock symbols:`, stockSymbols);
    }
  ...
  const forexWatchlist: WatchlistItem[] = forexSymbols.map((symbol) => {
    ...
    console.log(`Processing forex symbol ${symbol}: position = ${position}, map has:`, positions.has(symbol));
      ...
      >
        ...
        <div className="grid grid-cols-[1.25fr_.95fr_1fr_1.25fr] gap-2 items-center">
          ...
          <div className="min-w-0 flex items-center gap-2">
            ...
            >
              {item.category === "STOCKS" ? item.symbol.charAt(0) : item.symbol.slice(0, 2)}
            </div>

            <div className="min-w-0">
              <div className={`truncate font-semibold text-[12px] ${isDarkMode ? "text-white" : "text-gray-900"}`}>
                {item.symbol}
              </div>
              <div className={`truncate text-[11px] ${isDarkMode ? "text-[#7f8898]" : "text-gray-500"}`}>
                {item.name}
              </div>
            </div>
          </div>
          ...
          <div className="text-right">
            <div className={`font-mono font-semibold text-[12px] ${isDarkMode ? "text-white" : "text-gray-900"}`}>
              {(() => {
                console.log('Displaying position for', item.symbol, ':', item.position);
                return formatShares(item.position ?? 0);
              })()}
            </div>
            <div className={`text-[11px] ${isDarkMode ? "text-[#7f8898]" : "text-gray-500"}`}>Shares</div>
          </div>
          ...
          <div className="text-right">
            ...
            <div className={`text-[11px] ${isDarkMode ? "text-[#7f8898]" : "text-gray-500"}`}>Price</div>
          </div>

          {/* Change */}
          <div className="text-right">
            <div className={`font-mono font-semibold text-[12px] ${up ? "text-emerald-400" : "text-rose-400"}`}>
              {up ? "+" : ""}
              {changeText}
            ...
            <div className={`text-[11px] ${isDarkMode ? "text-[#7f8898]" : "text-gray-500"}`}>Change</div>
          </div>
        </div>
      </button>
    );
  ...
  const Section = ({
    ...
    return (
      <div className={`border-b ${isDarkMode ? "border-[#2a2e39]" : "border-gray-200"}`}>
        <button
          onClick={onToggle}
        ...
        >
          <div className="flex items-center gap-2">
            <span className={`text-[11px] font-semibold tracking-wider ${isDarkMode ? "text-[#b7bfcc]" : "text-gray-600"}`}>
              {title}
            </span>
            <span className={`text-[10px] px-1.5 py-0.5 rounded ${isDarkMode ? "bg-[#1e222d] text-[#9aa4b2]" : "bg-gray-100 text-gray-600"}`}>
              {items.length}
            </span>
          </div>
        ...
        {expanded && (
          <div className={`${isDarkMode ? "bg-[#131722]" : "bg-white"} max-h-[380px] overflow-y-auto`}>
            {loading ? (
              <div className={`px-3 py-3 text-center text-[12px] ${isDarkMode ? "text-[#7f8898]" : "text-gray-500"}`}>
                Loading...
              </div>
            ) : (
  ...
  return (
    <div className={`w-full h-full flex flex-col ${isDarkMode ? "text-white" : "text-gray-900"}`}>
      {/* Header */}
      <div className={`border-b px-3 py-3 ${isDarkMode ? "border-[#2a2e39]" : "border-gray-200"}`}>
        <div className="flex items-center justify-between">
          <h3 className={`font-semibold text-sm ${isDarkMode ? "text-white" : "text-gray-900"}`}>Watchlist</h3>

          <div className="flex items-center gap-2">
            <select
              className={`border rounded px-2 py-1 text-xs ${
                isDarkMode ? "bg-[#1e222d] border-[#2a2e39] text-white" : "bg-white border-gray-300 text-gray-900"
              }`}
            >
        ...
        {/* Column hint (nhẹ, gọn) */}
        <div className={`mt-2 grid grid-cols-[1.25fr_.95fr_1fr_1.25fr] gap-2 text-[11px] ${
